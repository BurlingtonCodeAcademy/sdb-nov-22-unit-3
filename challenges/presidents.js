let presidentsArray = [
  ...
  { first: "Joe", last: "Biden", year: 1942, passed: undefined },
];

//? How many distinct presidests have we had?
// console.log("How many Presidents?", presidentsArray.length);
...
let deadPresidents = presidentsArray.filter(
  (president) => president.passed !== undefined
);
// console.log(deadPresidents);

// ? What if you want the first and last names alive ones in an array

let namedPresidents = livingPresidents.map(
  (president) => `${president.first} ${president.last}`
);

// console.log(namedPresidents);

//? using forEach loop through the presidents and have it console.log
...
deadPresidents.forEach((president) => {
  //  Code goes in here with a console.log()
  let { passed, year } = president;

  //   passed  === president.passed
  let age = passed - year;
  console.log(` was ${age} when he died`);
});
